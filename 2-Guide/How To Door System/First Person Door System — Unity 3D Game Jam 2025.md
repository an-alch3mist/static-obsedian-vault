...
---
### NavMesh & AI Pathing When Door Opens/Closes
**Problem:** NavMesh agents don't automatically repath when doors change state.
**Recommended Approach:** Use `NavMeshObstacle` component with **carving enabled**:
```csharp
// Add to DoorBase.cs in SetState()
...
protected void SetState(DoorState newState)
{
    ...
    if (navObstacle != null)
    {
        navObstacle.carving = (newState == DoorState.Closed || newState == DoorState.Locked);
        navObstacle.enabled = navObstacle.carving;
    }
}
```

**Why This Works:**
- `NavMeshObstacle.carving = true` → agents will path _around_ door
- `carving = false` → agents will path _through_ door
- Happens **instantly** without rebaking NavMesh
- Works off main thread

**Alternative:** If using NavMesh baking, call `NavMesh.AddObstacle()` / `NavMesh.RemoveObstacle()` in `SetState()`, but this is **not recommended** (performance hit).
...
private IEnumerator AutoCloseRoutine()
{
    ...
    int attempts = 0;
    while (attempts < maxAutoCloseRetries)
    {
        if (!IsObstructed())
        {
            TryClose();
...
**Edge Case: Player Takes Damage?**

- If crushing is undesirable, teleport player to nearest valid position:

```csharp
// Add to DoorBase.cs
protected void NudgeObstruction()
{
    Collider[] hits = Physics.OverlapBox(blockingCollider.bounds.center, ...);
    foreach (Collider hit in hits)
    {
        if (hit.CompareTag("Player"))
        {
            // Nudge player 1m away from door
            Vector3 nudgeDir = (hit.transform.position - transform.position).normalized;
            hit.transform.position += nudgeDir * 1f;
        }
    }
}
```
...
#### Option 1: Disable Blocking Collider During Transition

```csharp
// DoorBase.cs - PerformOpen()
protected override IEnumerator PerformOpen()
{
    blockingCollider.enabled = false; // No blocking during animation
    yield return StartCoroutine(PerformOpen());
    blockingCollider.enabled = false; // Door fully open, no collision
}
```
...
**Problem:** Player saves game while door is opening → reload causes door to snap to final state.

**Solution:** Serialize animation progress and resume:

```csharp
// DoorBase.cs - Enhanced SaveState()
[System.Serializable]
public class DoorSaveData
{
    ...
    public float animationProgress; // 0–1 range
}
...
protected override void LoadState()
{
    ...
    if (data == null || data.doorId != gameObject.name) return;
    
    currentState = data.state;
    isLocked = data.isLocked;
    
    // Resume animation at saved progress
    if (data.state == DoorState.Opening || data.state == DoorState.Closing)
    {
        animator.Play(data.state == DoorState.Opening ? "doorOpeningAnim" : "doorClosingAnim", 0, data.animationProgress);
        
        // Restart coroutine to complete animation
        if (data.state == DoorState.Opening)
            openRoutine = StartCoroutine(OpenRoutine());
        else
...
**Idempotency:** If `LoadState()` is called multiple times (e.g., scene reload), check if already loaded:
...
### 2. **Networked Multiplayer Doors**

**Approach:** Add `NetworkBehaviour` wrapper:

```csharp
// DoorNetwork.cs (Mirror/Netcode)
public class DoorNetwork : NetworkBehaviour
{
    ...
    {
        door.TryOpen();
        RpcSyncState(door.State);
    }
    
    [ClientRpc]
    void RpcSyncState(DoorState newState)
    {
        // Sync animation state on all clients
    }
}
```
